...
class ScriptFile(MTbp.BIRRP_Parameters):
    ...
    def _validate_fn_arr(self):
        ...
        if type(self.fn_arr[0]) is not np.ndarray:
            raise MTbp.Script_File_Error('Input fn_arr elements should be '
                                         'numpy arrays with '
                                         'dtype {0}'.format(self._fn_dtype))

        if self.fn_arr[0].dtype is not self._fn_dtype:
            raise MTbp.Script_File_Error('fn_arr.dtype needs to '
                                         'be {0}'.format(self._fn_dtype))
            ...
            for ff, fn_arr in enumerate(self.fn_arr):
                ...
                for cc in self.comp_list:
                    ...
                    try:
                        ...
                        print('Something amiss with remote reference')
...
def core_edi(datadir):
    ...
    savepath = datadir
    allfiles = glob.glob(datadir+r'\s*.edi')
...
#                        )
    ...
    allfiles = glob.glob(save_path+r'\*.iter')
...
#    savefn = r"C:\Work\data\MT\biirp\test"
    scriptfn = r'C:\Work\data\mtd\birrp.txt'

#    MTbp.runbirrp2in2out_simple(birrp_exe, stationname, ts_dir,
#                                coherence_th, None, None,
#                                starttime, endtime)

    bs = ScriptFile(scriptfn)

    fn_dtype = bs._fn_dtype
...
#                         ('end_dt', 'S19')])
    ...
    fn_arr = np.zeros(4, dtype=fn_dtype)

    fn_arr[0]['fn'] = r'C:\Work\data\mtd\RM0105_090525230300.EX'
    fn_arr[0]['nread'] = 1
    fn_arr[0]['nskip'] = 0
    fn_arr[0]['calibration_fn'] = r''
    fn_arr[0]['rr'] = False
    fn_arr[0]['rr_num'] = 0
    fn_arr[0]['start_dt'] = r''  # start date ?
    fn_arr[0]['end_dt'] = r''  # end date ?
    fn_arr[0]['comp'] = 'ex'

    fn_arr[1]['nread'] = 1
    fn_arr[1]['nskip'] = 0
    fn_arr[1]['calibration_fn'] = r''
    fn_arr[1]['rr'] = False
    fn_arr[1]['rr_num'] = 0
    fn_arr[1]['start_dt'] = r''  # start date ?
    fn_arr[1]['end_dt'] = r''  # end date ?
    fn_arr[1]['comp'] = r'ey'
    fn_arr[1]['fn'] = r'C:\Work\data\mtd\RM0105_090525230300.EY'

    fn_arr[2]['nread'] = 1
    fn_arr[2]['nskip'] = 0
    fn_arr[2]['calibration_fn'] = r''
    fn_arr[2]['rr'] = False
    fn_arr[2]['rr_num'] = 0
    fn_arr[2]['start_dt'] = r''  # start date ?
    fn_arr[2]['end_dt'] = r''  # end date ?
    fn_arr[2]['comp'] = r'hx'
    fn_arr[2]['fn'] = r'C:\Work\data\mtd\RM0105_090525230300.BX'

    fn_arr[3]['nread'] = 1
    fn_arr[3]['nskip'] = 0
    fn_arr[3]['calibration_fn'] = r''
    fn_arr[3]['rr'] = False
    fn_arr[3]['rr_num'] = 0
    fn_arr[3]['start_dt'] = r''  # start date ?
    fn_arr[3]['end_dt'] = r''  # end date ?
    fn_arr[3]['comp'] = r'hy'
    fn_arr[3]['fn'] = r'C:\Work\data\mtd\RM0105_090525230300.BY'

#    bs = MTbp.ScriptFile(scriptfn)
    bs.fn_arr = [fn_arr]
    bs.deltat = 0.002
#    bs.nfft = 2**15
