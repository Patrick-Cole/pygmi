    ...
    def remove_ss(self, reduce_res_factor_x=1., reduce_res_factor_y=1.):
        ...
        if np.iterable(reduce_res_factor_x) == 0:
            try:
                ...
                self._logger.error(
                    'reduce_res_factor_x must be a valid numbers')
                return
            ...
            try:
                ...
                self._logger.error(
                    '"reduce_res_factor_x" must be valid numbers')
                return

        if len(lo_x_factors) != len(self.z):
            self._logger.error(
                'Wrong number Number of reduce_res_factor_x - need {0}'.format(len(self.z)))
            return
        ...
        if np.iterable(reduce_res_factor_y) == 0:
            try:
                ...
                self._logger.error(
                    '"reduce_res_factor_y" must be a valid numbers')
                return
            ...
            try:
                ...
                self._logger.error(
                    '"reduce_res_factor_y" must be valid numbers')
                return
