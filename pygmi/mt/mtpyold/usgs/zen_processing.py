...
import pygmi.mt.mtpyold.processing.birrp as birrp
import pygmi.mt.mtpyold.utils.configfile as mtcfg
import pygmi.mt.mtpyold.utils.exceptions as mtex
import pygmi.mt.mtpyold.imaging.plotnresponses as plotnresponses
import pygmi.mt.mtpyold.imaging.plotresponse as plotresponse
import pygmi.mt.mtpyold.usgs.zen as zen
import pygmi.mt.mtpyold.core.edi as mtedi
import pygmi.mt.mtpyold.core.ts as mtts
from pygmi.mt.mtpyold.usgs import z3d_collection as zc
...
class SurveyConfig(object):
    ...
    def from_df(self, z3d_df):
        ...
        self.e_xaxis_length = s_df[s_df.component == 'ex'].dipole_length.mode()[
            0]
        self.e_yaxis_azimuth = s_df[s_df.component == 'ey'].azimuth.mode()[0]
        self.e_yaxis_length = s_df[s_df.component == 'hx'].dipole_length.mode()[
            0]
...
class Z3D2EDI(object):
    ...
    Example
    ------------
        >>> import pygmi.mt.mtpyold.usgs.zen_processing as zp
        >>> zp_obj = zp.Z3D2EDI()
    ...
    def write_script_files(self, birrp_arr_dict, save_path=None,
        ...
        .. seealso:: pygmi.mt.mtpyold.processing.birrp
    ...
    def plot_responses(self, edi_fn_list=None):
        ...
        :return: plot object
        :rtype: pygmi.mt.mtpyold.imaging.plotnresponse.PlotMultipleResponses
    ...
    def process_data(self, df_fn=None, df_list=[4096, 256, 4], plot=True,
        ...
        :param birrp_param_dict: dict(birrp_param: value)
                                 ...
                                 pygmi.mt.mtpyold.processing.birrp.write_script_file
        ...
        for edi_fn in edi_fn_list:
            ...
            if sr_key in list(sr_dict.keys()):
                try:
                    ...
                    data_arr['z'][count:count +
                                  len(f_index[0])] = edi_obj.Z.z[f_index]
                    ...
                    if edi_obj.Tipper.tipper is not None:
                        data_arr['tipper'][count:count +
                                           len(f_index[0])] = edi_obj.Tipper.tipper[f_index]
                        data_arr['tipper_err'][count:count +
                                               len(f_index[0])] = edi_obj.Tipper.tipper_err[f_index]
                ...
                pass
                print(
                    'WARNING: {0} was not in combining dictionary'.format(sr_key))
...
def compute_mt_response(survey_dir, station='mt000', copy_date=None,
    ...
    Returns
    -----------------
        **rp_plot** : pygmi.mt.mtpyold.imaging.plotnresponses object
