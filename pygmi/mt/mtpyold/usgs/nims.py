...
from pygmi.mt.mtpyold.core import ts
...
class GPS(object):
    ...
    def _validate_latitude(self, latitude_str, hemisphere_str):
        ...
        if hemisphere_str.lower() not in ['n', 's']:
            raise GPSError('Latitude hemisphere {0} not understood'.format(
    ...
    def _validate_longitude(self, longitude_str, hemisphere_str):
        ...
        if hemisphere_str.lower() not in ['e', 'w']:
            raise GPSError('Longitude hemisphere {0} not understood'.format(
                hemisphere_str.upper()))
        try:
            float(longitude_str)
        except ValueError:
    ...
    def latitude(self):
        ...
        if self._latitude is not None and self._latitude_hemisphere is not None:
            ...
            lat = float(self._latitude[0:index]) + \
    ...
    def longitude(self):
        ...
        if self._longitude is not None and self._longitude_hemisphere is not None:
            ...
            lon = float(self._longitude[0:index]) + \
    ...
    def time_stamp(self):
        ...
        try:
            ...
            logging.error(
                'GPSError: bad date string {0}'.format(self.gps_string))
            return None
...
class NIMS(NIMSHeader):
    ...
    def find_sequence(self, data_array, block_sequence=None):
        ...
        slices = [np.s_[ii:n_data - n_sequence + 1 + ii]
    ...
    def _locate_duplicate_blocks(self, sequence):
        ...
        for dup in duplicates:
            ...
            dup_dict['ts_index_1'] = dup * \
                self.sampling_rate + self.sampling_rate
            dup_dict['ts_index_2'] = (dup + 1) * self.sampling_rate
            dup_dict['ts_index_3'] = (
                dup + 1) * self.sampling_rate + self.sampling_rate
    ...
    def _locate_timing_gaps(self, stamps):
        ...
        for ii, stamp in enumerate(stamps[1:], 1):
            ...
            if stamp._date == '010180':
                diff_arr[ii] = -666
                continue
            time_diff = (stamp.time_stamp -
                         stamp_01.time_stamp).total_seconds()
