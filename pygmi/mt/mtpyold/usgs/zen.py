...
import pygmi.mt.mtpyold.imaging.plotspectrogram as plotspectrogram
        ...
        for h_str in header_list:
            ...
            # need to adjust for older versions of z3d files
                ...
                for hh in h_str.split(','):
                    ...
                    m_key = m_key.strip().lower().replace(
                        ' ', '_').replace('/', '').replace('.', '_')
...
class Z3DMetadata(object):
    ...
    def read_metadata(self, fn=None, fid=None):
        ...
        if self.fn is None and self.fid is None:
            ...
            if self.fid is not None:
                self.fid.seek(self._header_length +
        ...
        while self.find_metadata == True:
            try:
                test_str = self.fid.read(
                    self._metadata_length).decode().lower()
    ...
    * gps_dtype is formated as np.dtype([('flag0', np.int32),
    ...
    def _read_schedule(self, fn=None, fid=None):
        ...
        if self.header.old_version:
            ...
            self.schedule.Date = dt_str.split(',')[0]
            self.schedule.Time = dt_str.split(',')[1]
            year, month, day = [int(dd)
                                for dd in self.schedule.Date.split('-')]
            hour, minute, second = [int(dd)
                                    for dd in self.schedule.Time.split(':')]
    ...
    def read_z3d(self, Z3Dfn=None):
        ...
        try:
            ...
            raise ZenGPSError(
                "Data is bad, cannot open file {0}".format(self.fn))
    ...
    def check_start_time(self):
        ...
        print('    1st good stamp was {0} (UTC time)'.format(
            zen_start_utc.isoformat()))
        print('    difference of {0:.2f} seconds'.format(
    ...
    def validate_gps_time(self):
        ...
        for ii in range(len(t_diff) - 1):
            t_diff[ii] = self.gps_stamps['time'][ii] - \
                self.gps_stamps['time'][ii + 1]
    ...
    def apply_adaptive_notch_filter(self, notch_dict={'notches': np.arange(60, 1860, 60),
        ...
        frequency.
        see pygmi.mt.mtpyold.processing.filter.adaptive_notch_filter
        Arguments
        -------------
    ...
    def write_ascii_mt_file(self, save_fn=None, fmt='%.8e', notch_dict=None,
        ...
        if os.path.isfile(self.fn_mt_ascii) == True:
            ...
            self.zen_schedule = dateutil.parser.parse(
                self.ts_obj.start_time_utc)
        ...
        print('INFO: Wrote mtpy timeseries file to {0}'.format(
            self.fn_mt_ascii))
    ...
    def plot_spectrogram(self, time_window=2**8, time_step=2**6, s_window=11,
        ...
        Returns:
        ---------
            **ptf** : pygmi.mt.mtpyold.imaging.plotspectrogram.PlotTF object
        """
    ...
    def make_schedule(self, df_list, df_length_list, repeat=5, t1_dict=None):
        ...
        if t1_dict is not None:
            ...
            df_length_list = np.append(
                df_length_list[kk:], df_length_list[:kk])
