...
from pygmi.mt.mtpyold.modeling.modem import Model, Data
from pygmi.mt.mtpyold.utils import gis_tools, convert_modem_data_to_geogrid
...
gce, gcn, gcz = [np.mean([arr[:-1], arr[1:]], axis=0)
...
for ascfn in ascfilelist:
    ...
    newgridres = 10**interpfunc(
        np.vstack([newgridx.flatten(), newgridy.flatten()]).T).reshape(newgridx.shape)
...
#            else:
#                break
    np.savetxt(os.path.join(wd, ascfn), newgridres,
               header=header, comments='', fmt='%.3e')
