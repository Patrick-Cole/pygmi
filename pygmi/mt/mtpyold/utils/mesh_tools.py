...
def get_padding_cells(cell_width, max_distance, num_cells, stretch):
    ...
    for ii in range(num_cells):
        ...
        mult_pad = np.round((cell_width * stretch) *
                            ((1 - stretch**(ii + 1)) / (1 - stretch)), -2)
...
def get_padding_from_stretch(cell_width, pad_stretch, num_cells):
    ...
    """
    nodes = np.around(cell_width * (np.ones(num_cells) *
                      pad_stretch)**np.arange(num_cells), -2)
...
def get_padding_cells2(cell_width, core_max, max_distance, num_cells):
    ...
    cells = np.around(np.logspace(np.log10(core_max),
                      np.log10(max_distance), num_cells), -2)
    cells -= core_max
