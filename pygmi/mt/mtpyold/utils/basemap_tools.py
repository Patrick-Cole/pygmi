...
from pygmi.mt.mtpyold.modeling.modem import Data
...
def compute_map_extent_from_modem_data(stations_obj, buffer=None, buffer_factor=0.1):
    ...
    lonMin, lonMax, latMin, latMax = get_latlon_extents_from_modem_data(
        stations_obj)

    # compute buffer
    if buffer is None:
        buffer = max([(lonMax - lonMin) * buffer_factor,
                     (latMax - latMin) * buffer_factor])
...
def add_basemap_frame(basemap, tick_interval=None, coastline_kwargs={}, states_kwargs={},
    ...
    basemap.drawmeridians(np.arange(np.floor(basemap.lonmin), np.ceil(
        basemap.lonmax), tick_interval), labels=mlabels)
    basemap.drawparallels(np.arange(np.floor(basemap.latmin), np.ceil(
        basemap.latmax), tick_interval), labels=plabels)
