...
import pygmi.mt.mtpyold.modeling.occam2d as o2d
from pygmi.mt.mtpyold.utils import mesh_tools, gis_tools, filehandling
...
def plot(m2d_profile, profile_elevation, site_locations, site_elevations, site_names, figsize=None):
    ...
    ax.plot(site_locations, site_elevations, color='r',
            marker='*', linestyle='None', label='Sites')
...
def get_profile_specs(o2d_data, site_easts, site_norths):
    ...
    Parameters
    ----------
    m2d_profile : pygmi.mt.mtpyold.modeling.occam2d.Data
    ...
    utm_zone = gis_tools.get_utm_zone(
        projected_origin[0], projected_origin[1])[2]
...
def generate_profile_line(site_easts, site_norths, x0, y0, x1, y1, elevation_sample_n):
    ...
    o2d_easts = np.delete(np.linspace(
        x0, x1, elevation_sample_n, endpoint=False), 0)
    o2d_norths = np.delete(np.linspace(
        y0, y1, elevation_sample_n, endpoint=False), 0)
...
def occam2d_to_mare2dem(o2d_data, rot_o2d_data, surface_file, elevation_sample_n=300,
    ...
    Parameters
    ----------
    o2d_data : pygmi.mt.mtpyold.modeling.occam2d.Data
        Occam2D Data object that has been initialised with EDI data and
        *not* rotated to strike angle.
    rot_o2d_data : pygmi.mt.mtpyold.modeling.occam2d.Data
    ...
    prof_easts, prof_norths = \
        generate_profile_line(site_easts_orig, site_norths_orig,
                              x0, y0, x1, y1, elevation_sample_n)
    # Rotated profile
...
def write_elevation_file(m2d_profile, profile_elevation, savepath=None):
    ...
    elevation_model = np.stack(
        (m2d_profile, profile_elevation), axis=1).astype(np.float64)
    if savepath is None:
...
def write_mare2dem_data(o2d_filepath, site_locations, site_elevations, site_names,
    ...
    type_conversion = {1: 123, 2: 104, 3: 133,
                       4: 134, 5: 125, 6: 106, 9: 103, 10: 105}
    ...
    data_df = pd.DataFrame(
        (types, freqs, sites, sites, datums, errors), dtype=np.object).T
    ...
    if isinstance(solve_statics, bool):
        statics = np.ones(site_locations.shape,
                          dtype=np.int8) if solve_statics else zero_ints
    else:
    ...
    recv_df.columns = ['X', 'Y', 'Z', 'Theta', 'Alpha',
                       'Beta', 'Length', 'SolveStatic', 'Name']
    recv_str = list(recv_df.to_string(
