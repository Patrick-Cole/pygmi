...
from pygmi.mt.mtpyold.imaging import mtplottools as mtplottools
from pygmi.mt.mtpyold.modeling.modem import Data, Residual
                ...
                for resp_obj in self.resp_object:
                    ...
                    if self.plot_tipper == True:
                        rertx = mtplottools.plot_errorbar(axtxr,
                                                          period[ntx],
                                                          resp_t_obj.tipper[ntx,
                                                                            0, 0].real,
                                                          None,
                                                          **kw_xx)
                        rerty = mtplottools.plot_errorbar(axtyr,
                                                          period[nty],
                                                          resp_t_obj.tipper[nty,
                                                                            0, 1].real,
                                                          None,
                                                          **kw_yy)
...
#                              prop={'size': max([self.font_size / (nr + 1), 5])})
            if self.plot_style == 2:
                if self.plot_component == 2:
                    ...
                    legend_ax_list = self.ax_list[0:int(
                        self.plot_component / 2)]
                    if plot_tipper == True:
                        if self.plot_component == 2:
                            legend_ax_list.append(self.ax_list[2])
                        elif self.plot_component == 4:
        ...
        for ii, si in enumerate(s_index_list):
            ...
            if self.plot_z:
                gs = gridspec.GridSpec(4, 1, height_ratios=[
                                       0.85, 0.85, 0.3, 0.3], hspace=0.1, left=left)
            else:
                gs = gridspec.GridSpec(4, 1, height_ratios=[
                                       1, 0.7, 0.3, 0.3], hspace=0.1, left=left)
            axr = fig.add_subplot(
            ...
            for di in range(2):
                ...
                for i in range(2):
                    for j in range(2):
                        ...
                        axr.errorbar(1. / zObj.freq[nonzero], data1[nonzero][:, i, j],
                                     yerr=data1err[nonzero][:, i, j], label='Res' + 'XY'[i] + 'XY'[j], **kwargs)
                        axp.errorbar(1. / zObj.freq[nonzero], data2[nonzero][:, i, j],
                                     yerr=data2err[nonzero][:, i, j], label='Phs' + 'XY'[i] + 'XY'[j], **kwargs)
                    ...
                    axt[i].errorbar(1. / tObj.freq[nonzerot], tObj.tipper.real[nonzerot]
                                    [:, 0, i], tipper_err, label='Tip' + 'XY'[i] + 'R', **kwargs)
                    kwargs['color'] = color_tip[di, 1]
                    axt[i].errorbar(1. / tObj.freq[nonzerot], tObj.tipper.imag[nonzerot]
                                    [:, 0, i], tipper_err, label='Tip' + 'XY'[i] + 'I', **kwargs)
            ...
            if self.tipper_limits is not None:
                yticks = [np.round(self.tipper_limits[0] * 0.75, 1),
                          0, np.round(self.tipper_limits[1] * 0.75, 1)]
                for axti in axt:
