...
import pygmi.mt.mtpyold.analysis.pt as pt
from pygmi.mt.mtpyold.core import mt as mt
from pygmi.mt.mtpyold.core import z as mtz
from pygmi.mt.mtpyold.modeling import ws3dinv as ws
from pygmi.mt.mtpyold.utils import gis_tools as gis_tools
from pygmi.mt.mtpyold.utils.mtpy_decorator import deprecated
from pygmi.mt.mtpyold.utils.mtpylog import MtPyLog

from pygmi.mt.mtpyold.modeling.modem.exception import ModEMError, DataError
from pygmi.mt.mtpyold.modeling.modem.station import Stations
from pygmi.mt.mtpyold.modeling.modem.model import Model
...
class Data(object):
    ...
    ..note: :: the data is interpolated onto the given periods such that all
               ...
               See pygmi.mt.mtpyold.core.mt.MT.interpolate for more details
    ...
    mt_dict                dictionary of pygmi.mt.mtpyold.core.mt.MT objects with keys
                           being station names
    ...
    convert_ws3dinv_data_file  convert a ws3dinv file to ModEM fomrat,
                               **Note** this doesn't include tipper data and
                               ...
                               output by pygmi.mt.mtpyold.modeling.ws3dinv
    ...
    :Example 1 --> create inversion period list: ::
        ...
        >>> import os
        >>> import pygmi.mt.mtpyold.modeling.modem as modem
    ...
    :Example 2 --> set inverions period list from data: ::
        ...
        >>> import os
        >>> import pygmi.mt.mtpyold.core.mt
        >>> import pygmi.mt.mtpyold.modeling.modem as modem
        >>> edi_path = r"/home/mt/edi_files"
        >>> edi_list = [os.path.join(edi_path, edi) \
                        for edi in os.listdir(edi_path)\
        ...
        for key in list(kwargs.keys()):
            ...
            if key != 'rotation_angle':
                if hasattr(self, key):
                    ...
                    self._logger.warn(
                        "Argument {}={} is not supported thus not been set.".format(key, kwargs[key]))
        ...
        if self.period_buffer is not None:
            try:
                ...
                if self.period_buffer < 0.:
                    self._logger.warn(
                        "Period buffer must be > 0, setting to None")
                    self.period_buffer = None
...
#            return
        ...
        for ii, s_key in enumerate(sorted(self.mt_dict.keys())):
            ...
            if type(self.period_buffer) in [float, int]:
                ...
                for iperiod in interp_periods:
                    ...
                    nearestdperiod = dperiods[difference == np.amin(
                        difference)][0]
            ...
            if len(interp_periods) > 0:  # not empty
                interp_z, interp_t = mt_obj.interpolate(
                    1. / interp_periods, period_buffer=self.period_buffer, bounds_error=False)  # )
                # set rotation angle
                interp_z.rotation_angle = self.rotation_angle * \
                    np.ones(len(interp_z.z))
                interp_t.rotation_angle = self.rotation_angle * \
                    np.ones(len(interp_t.tipper))
    ...
    def compute_inv_error(self):
        ...
        if 'floor' in self.error_type_tipper:
            t_index = np.where(
                self.data_array['tip_err'] < self.error_value_tipper)
            ...
            raise DataError("Unsupported error type (tipper): {}".format(
            ...
            for ff in range(max([self._t_shape[0], self._z_shape[0]])):
                ...
                for ei, error_type_z in enumerate(error_type_z_list.flatten()):
                    if 'egbert' in error_type_z:
                        ...
                        d2d = d.reshape((2, 2))
                        err[ei] = err_value * \
                            np.abs(np.linalg.eigvals(d2d)).mean()
                ...
                if (error_type_z_list.size == 1):
                    ...
                    for ei in np.arange(error_type_z_list.size):
                        ...
                        if (err.shape[1] > 1):
                            self.data_array['z_inv_err'][ss,
                                                         ff, ix, iy] = err[ei, ix, iy]
                        else:
                            self.data_array['z_inv_err'][ss,
                                                         ff, ix, iy] = err[ei, 0, 0]
                        # end if
    ...
    def write_data_file(self, save_path=None, fn_basename=None,
        ...
        if fill:
            ...
            new_edi_dir = os.path.join(self.save_path, 'new_edis')
        ...
        for inv_mode in self.inv_mode_dict[self.inv_mode]:
            if 'impedance' in inv_mode.lower():
                ...
                n_sta = len(np.nonzero(
                    np.abs(self.data_array['z']).sum(axis=(1, 2, 3)))[0])
                n_per = len(np.nonzero(
                    np.abs(self.data_array['z']).sum(axis=(0, 2, 3)))[0])
                ...
                d_lines.append('> []\n')
                n_sta = len(np.nonzero(
                    np.abs(self.data_array['tip']).sum(axis=(1, 2, 3)))[0])
                n_per = len(np.nonzero(
                    np.abs(self.data_array['tip']).sum(axis=(0, 2, 3)))[0])
            else:
                # maybe error here
                ...
                for ff in range(self.data_array['z'].shape[1]):
                    for comp in self.inv_comp_dict[inv_mode]:
                        ...
                        if zz.real != 0.0 and zz.imag != 0.0 and zz.real != 1e32 and zz.imag != 1e32:
                            ...
                            abs_err = self.data_array['{0}_inv_err'.format(
                                c_key)][ss, ff, z_ii, z_jj]
                            ...
                            dline = ''.join(
                                [per, sta, lat, lon, nor, eas, ele, com, rea, ima, abs_err, '\n'])
    ...
    def _impedance_components_error_meansqr(self, c_key, ss, z_ii, z_jj):
        ...
        """
        abs_err = np.mean(
            np.square(self.data_array[ss][c_key + '_err'][:, z_ii, z_jj]))
        return abs_err
    ...
    def convert_ws3dinv_data_file(self, ws_data_fn, station_fn=None,
        ...
        ------------
            ...
            **station_fn** : string
                             ...
                             pygmi.mt.mtpyold.modeling.ws3dinv. Or you can create one using
                             pygmi.mt.mtpyold.modeling.ws3dinv.WSStation
    ...
    def convert_modem_to_ws(self, data_fn=None, ws_data_fn=None,
        ...
        ws_data.data['z_data_err'][:, :,
                                   :] = self.data_array['z_err'] * (1 + 1j)
        ...
        for dd in data_list:
            ...
            if dd[7].find('Z') == 0:
                ...
                if self.wave_sign_impedance == '+':
                    ...
                    raise DataError("Incorrect wave sign \"{}\" (impedance)".format(
                        self.wave_sign_impedance))
            ...
            # fill in tipper with appropriate values
                ...
                if self.wave_sign_tipper == '+':
                    data_dict[dd[1]].Tipper.tipper[p_index,
                                                   ii, jj] = dd[8] + 1j * dd[9]
                elif self.wave_sign_tipper == '-':
                    data_dict[dd[1]].Tipper.tipper[p_index,
                                                   ii, jj] = dd[8] - 1j * dd[9]
                else:
                    raise DataError("Incorrect wave sign \"{}\" (tipper)".format(
                        self.wave_sign_tipper))
    ...
    def change_data_elevation(self, model_obj, data_fn=None, res_air=1e12):
        ...
        for s_arr in s_locations:
            e_index = np.where(model_obj.grid_east >=
                               s_arr['rel_east'])[0][0] - 1
            n_index = np.where(model_obj.grid_north >=
                               s_arr['rel_north'])[0][0] - 1
            z_index = np.where(
                model_obj.res_model[n_index, e_index, :] < res_air * .9)[0][0]
            s_index = np.where(
                self.data_array['station'] == s_arr['station'])[0][0]
    ...
    def project_stations_on_topography(self, model_object, air_resistivity=1e12):
        ...
        for sname in self.station_locations.station_locations['station']:
            ss = np.where(
                self.station_locations.station_locations['station'] == sname)[0][0]
    ...
    def compute_phase_tensor(self, datfile, outdir):
        ...
        for period_num in range(num_periods):
            ...
            for num_site in range(num_sites):
                ...
                arow = [freq, site_label, site_lat, site_long,
                        this_phimin, this_phimax, this_ellipticity, this_azimuth]
                # Done for this site
