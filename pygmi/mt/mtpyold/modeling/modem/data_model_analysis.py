...
class DataModelAnalysis(object):
    ...
    def get_slice_data(self, slice_location):
        ...
        logger.debug(
            "the slice index number= %s and the actual location is %s", sno, actual_location)
    ...
    def create_csv(self, csvfile='tests/temp/Resistivity.csv'):
        ...
        csv_header = ['X', 'Y', 'Z', 'Log_Resisitivity',
                      'StationName', 'StationX', 'StationY', 'Lat', 'Long']
        ...
        for zslice in z_cell_centres:
            ...
            for i in range(len(X) - 1):
                for j in range(len(Y) - 1):
                    ...
                    st = stationd.get((i, j), None)
                    if st is not None:
                        arow = [X[i], Y[j], Z_location, res[j, i],
                                st[0], st[1], st[2], st[3], st[4], i, j]
                        csvrows.append(arow)
