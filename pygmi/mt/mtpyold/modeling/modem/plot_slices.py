...
from pygmi.mt.mtpyold.modeling.modem.data import Data
from pygmi.mt.mtpyold.modeling.modem.data import Model
from pygmi.mt.mtpyold.utils import exceptions as mtex, basemap_tools
from pygmi.mt.mtpyold.utils.gis_tools import get_epsg, epsg_project
from pygmi.mt.mtpyold.utils.calculator import nearest_index
from pygmi.mt.mtpyold.utils.mesh_tools import rotate_mesh

from pygmi.mt.mtpyold.imaging.seismic import Segy, VelocityModel
    ...
    * Extract arbitrary profiles (e.g. along a seismic line) from a model
    ...
    def get_slice(self, option='STA', coords=[], nsteps=-1, nn=1, p=4,
        ...
        if (option == 'STA' or option == 'XY'):
            if (nsteps > -1):
                assert nsteps > 2, 'Must have more than 2 grid points in the ' \
            ...
            if (nsteps > -1):
                # create regular grid
                d = np.linspace(dst.min(), dst.max(), nsteps)
    ...
    def plot(self):
        ...
        radio = RadioButtons(self.ax_radio, ('N-E',  # (Depth Slice)
                                             # (North-south-aligned vertical profile)
                                             'N-Z',
    ...
    def export_slices(self, plane='N-E', indexlist=[], station_buffer=200, save=True):
        ...
        for ii in indexlist:
            ...
            mesh_plot = ax1.pcolormesh(xg,
                                       ...
                                       vmax=self.climits[1])
            # plot the stations
            if (self.station_east is not None and
                    self.plot_stations):
...
#                        ax1.plot(ee, nn, 'k.')
                    ...
                    sids = np.fabs(
                        self.grid_east[ii] - self.station_east) < station_buffer
                    ...
                    sids = np.fabs(
                        self.grid_north[ii] - self.station_north) < station_buffer
...
#                plt.close()
    ...
    def plot_resistivity_on_seismic(self, segy_fn, velocity_model=6000, pick_every=10, ax=None, cb_ax=None, percent_clip=99, alpha=0.5, **kwargs):
        ...
        ci = ax.contourf(mdist, mdepth, svalsClipped, 50,
                         ...
                         cmap='Greys', alpha=alpha / 3., rasterized=True)
...
if __name__ == '__main__':
    ...
    for f, fp in zip(figs, fpaths):
        ...
        newPos = [oldPos.x0, oldPos.y0,
                  oldPos.width / 2.0, oldPos.height / 2.0]
