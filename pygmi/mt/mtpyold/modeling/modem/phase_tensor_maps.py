...
from pygmi.mt.mtpyold.utils.gis_tools import epsg_project
from pygmi.mt.mtpyold.utils import basemap_tools
from pygmi.mt.mtpyold.modeling.modem import Data, Model
import logging
import traceback
...
class PlotPTMaps(mtplottools.MTEllipse):
    ...
    def __init__(self, data_fn=None, resp_fn=None, model_fn=None, **kwargs):
        ...
        if self.cb_tick_step is None:
            self.cb_tick_step = int(
                (self.ellipse_range[1] - self.ellipse_range[0]) / 2.)
    ...
    def _plot_induction_vectors(self, ax, m, periodIdx, ptarray='data', size_factor=10000,
        ...
        for sidx in range(len(self.data_obj.data_array)):
            if 'r' in plot_tipper:
                ax.arrow(x[sidx], y[sidx], size_factor * rx[sidx],
                         size_factor * ry[sidx], color='k', **kwargs_tip)
            if 'i' in plot_tipper:
                ax.arrow(x[sidx], y[sidx], size_factor * ix[sidx],
                         size_factor * iy[sidx], color='b', **kwargs_tip)
