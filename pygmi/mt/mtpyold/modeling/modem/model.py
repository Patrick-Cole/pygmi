...
from pygmi.mt.mtpyold.utils import mesh_tools as mtmesh, gis_tools as gis_tools, filehandling as mtfh
from pygmi.mt.mtpyold.utils.mtpylog import MtPyLog
from .exception import ModelError
import pygmi.mt.mtpyold.utils.gocad as mtgocad
...
class Model(object):
    ...
    Arguments
    -------------
        **station_object** : pygmi.mt.mtpyold.modeling.modem.Stations object
                            .. seealso:: pygmi.mt.mtpyold.modeling.modem.Stations
    ...
    def __init__(self, stations_object=None, data_object=None, **kwargs):
        ...
        self.title = 'Model File written by pygmi.mt.mtpyold.modeling.modem'
        self.res_scale = 'loge'
        ...
        for key in list(kwargs.keys()):
            if hasattr(self, key):
                ...
                self._logger.warn(
                    "Argument {}={} is not supportted thus not been set.".format(key, kwargs[key]))
    ...
    def nodes_z(self, nodes):
        nodes = np.array(nodes)
        self._nodes_z = nodes
        self.grid_z = np.array([nodes[0:ii].sum()
                               for ii in range(nodes.size)] + [nodes.sum()])
    ...
    def print_mesh_params(self, file=sys.stdout):
        # --> print out useful information
        print('-' * 15, file=file)
        print('\tNumber of stations = {0}'.format(
            len(self.station_locations.station)), file=file)
    ...
    def add_layers_to_mesh(self, n_add_layers=None, layer_thickness=None,
        ...
        self.res_model = np.vstack(
            [self.res_model[:, :, :n_add_layers].T, self.res_model.T]).T
        ...
        **inputs**
        ...
        for j in range(len(self.res_model)):
            for i in range(len(self.res_model[j])):
                ii = np.where((gcz > top_surface[j, i]) & (
                    gcz <= bottom_surface[j, i]))[0]
...
#        self._logger.debug("station grid index y: %s" % sgindex_y)
    ...
    def plot_sealevel_resistivity(self):
        ...
        if self.station_locations is None:
            ...
            plt.plot(self.station_locations.rel_east,
                     self.station_locations.rel_north, '.', color='k')
    ...
    def write_model_file(self, **kwargs):
        ...
        ----------------------
            ...
            **title** : string
                        ...
                        *default* is Model File written by pygmi.mt.mtpyold.modeling.modem
...
#        self.nodes_z = self.grid_z[1:] - self.grid_z[:-1]
    ...
    def interpolate_elevation2(self, surfacefile=None, surface=None, get_surfacename=False,
        ...
        **inputs**
        ...
        x0, y0 = self.station_locations.center_point.east[
            0], self.station_locations.center_point.north[0]
        ...
        print("Elevation data type and shape  *** ",
              type(elev_mg), elev_mg.shape, len(yg), len(xg))
    ...
    def add_topography_from_data(self, data_object, interp_method='nearest',
        ...
        Args:
            data_object (pygmi.mt.mtpyold.modeling.ModEM.data.Data): A ModEm data
    ...
    def add_topography_to_model2(self, topographyfile=None, surface=None,
        ...
        if self.n_air_layers is None or self.n_air_layers == 0:
            ...
            # get grid centre
            gcx, gcy = [np.mean([arr[:-1], arr[1:]], axis=0)
    ...
    def _get_xyzres(self, location_type, origin, model_epsg, model_utm_zone, clip):
        ...
        x, y, z = np.meshgrid(x[clip[0]:xsize - clip[0]],
                              y[clip[1]:ysize - clip[1]], z)
