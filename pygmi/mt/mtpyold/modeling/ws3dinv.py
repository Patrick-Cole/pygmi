...
:Making input files: ::
    ...
    >>> import pygmi.mt.mtpyold.modeling.ws3dinv as ws
    >>> import os
...
import pygmi.mt.mtpyold.modeling.winglink as wl
    ...
    def compute_errors(self):
        ...
        for d_arr in self.data:
            if self.z_err == 'data':
                ...
                d_arr['z_data_err'][ef_idx] = d_arr['z_data'][ef_idx] * \
                    self.z_err_floor
    ...
    def make_mesh(self):
        ...
        for ii in range(n_stations):
            ...
            xx = [nn for nn, xf in enumerate(east_grid)
                  if xf > (self.station_locations[ii]['east'] - self.cell_size_east) and
                  xf < (self.station_locations[ii]['east'] + self.cell_size_east)]
            ...
            yy = [mm for mm, yf in enumerate(north_grid)
                  if yf > (self.station_locations[ii]['north'] - self.cell_size_north) and
                  yf < (self.station_locations[ii]['north'] + self.cell_size_north)]
    ...
    def read_file(self):
        ...
        # --> read initial file
            ...
            for ii in range(len(self.res_list)):
                self.res_model[np.where(
                    self.res_model_int == ii + 1)] = rdict[ii + 1]
    ...
    def read_files(self):
        ...
        if self.station_fn is not None:
            if os.path.isfile(self.station_fn) == True:
                ...
                print('Could not find station file {0}'.format(
                    self.station_fn))
