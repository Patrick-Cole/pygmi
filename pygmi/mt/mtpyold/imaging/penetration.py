...
import pygmi.mt.mtpyold
import pygmi.mt.mtpyold.modeling.occam2d_rewrite as occam2d
...
class Depth2D(ImagingBase):
    ...
    def plot(self, tick_params={}, **kwargs):
        ...
        plt.xlabel('MT Penetration Depth Profile Over Stations.',
                   fontsize=fontsize)
...
class Depth3D(ImagingBase):
    ...
    def plot(self, fontsize=14, **kwargs):
        ...
        plot_station_id = kwargs.pop("plot_station_id", False)
        ...
        if check_period_values(periods, ptol=self._ptol) is False:
            ...
            if period0 < 1.0:
                ...
                self._period_fmt = str(
                    pygmi.mt.mtpyold.utils.calculator.roundsf(period0, 4))
            else:
            ...
            # from pygmi.mt.mtpyold.imaging.penetration_depth3d import reverse_colourmap
            ...
            plt.plot(station_points[:, 1],
                     station_points[:, 0], 'kv', markersize=6, )
            # add station id
            if plot_station_id:
            ...
            mycb.set_label(label='Penetration Depth ({})'.format(
                z_unit), size=fontsize)
...
def get_penetration_depth_by_period(mt_obj_list, selected_period, ptol=0.1, whichrho='det'):
    ...
    for mt_obj in mt_obj_list:
        ...
        print("********* The period-index=%s coressponding to the selected_period %s" %
              (per_index, selected_period))

        if abs(selected_period - per) > (selected_period) * ptol:
            print("************** Different the selected period =",
                  selected_period, per)
            # periods.append(np.nan)
            periods.append(selected_period)
            pen_depth.append(np.nan)
            _logger.warning("Nearest preiod {} on station {} was beyond tolerance of {} ".format(
    ...
    _logger.debug("The length of stations, periods, pen_depth, latlons: %s,%s,%s,%s ", len(
        stations), len(periods), len(pen_depth), len(latlons))
    ...
    def __init__(self, *args, **kwargs):
        if args is None:
            ParameterError.__init__(
                self, "please set zcomponent (rho) to either \"zxy\", \"zyx\" or \"det\"", **kwargs)
        else:
