...
class PlotStrike(object):
    ...
    ----------
        ...
        :param z_object: object of pygmi.mt.mtpyold.core.z.  If this is input be sure the
                          attribute z.freq is filled.  *default* is None

        :param mt_object: object of pygmi.mt.mtpyold.imaging.mtplot.MTplot
                         *default* is None
    ...
    def make_strike_array(self):
        ...
        self.max_per = np.amax([np.max(list(mm.keys()))
                               for mm in inv_list], axis=0)
        self.min_per = np.amin([np.min(list(mm.keys()))
                               for mm in pt_list], axis=0)
    ...
    def get_mode(self, st_hist):
        ...
        """
        s_mode = 90 - \
            st_hist[1][np.where(st_hist[0] == st_hist[0].max())[0][0]]
        s_mode %= 360
        ...
        if self.plot_type == 1:
            ...
            for jj, bb in enumerate(self._bin_range, 1):
                ...
                for aa, axh in enumerate(ax_list):
                    ...
                    axh.yaxis.set_major_locator(
                        MultipleLocator(self.ring_spacing))
