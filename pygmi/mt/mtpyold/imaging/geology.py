    ...
    def processLUT(self, lutfn, lut_delimiter=' '):
        ...
        if (lutfn is not None):
            ...
            for i, line in enumerate(lines):
                ...
                color = np.array(line.strip().split(
                    lut_delimiter)[1].split(','))
                try:
...
def main():
    ...
    polyDataShapefile = os.path.join(
        ModEM_files, 'NT_LithInterp_2500K_region.shp')
    lineDataShapefile = os.path.join(
        ModEM_files, 'NT_Fault_2500K_polyline.shp')
