...
class PlotResPhasePseudoSection(object):
    ...
    ----------
        ...
        **z_object** : list of class pygmi.mt.mtpyold.core.z.Z
                      object of pygmi.mt.mtpyold.core.z.  If this is input be sure the
                      attribute z.frequency is filled.  *default* is None

        **res_object_list** : list of class pygmi.mt.mtpyold.mtplottools.ResPhase
    ...
    -------------------
        ...
        *phase_cmap*: [ 'mt_yl2rd' | 'mt_bl2yl2rd' | 'mt_wh2bl' |
                      ...
                      colormaps from pygmi.mt.mtpyold.imaging.mtcolors, which are:
    ...
    ================ ==========================================================
        ...
        mt_list       list of pygmi.mt.mtpyold.mtplottools.MTplot instances
    ...
    def plot(self, show=True, get_rp_arrays=True):
        ...
        if self.plot_style == 'pcolormesh':
            ...
            ygrid_edges = 10**np.mean([np.log10(self.plot_period[1:]),
                                      np.log10(self.plot_period[:-1])], axis=0)
            ygrid = np.hstack(
...
#                                                 self.plot_period[-1]*1.1))
            ...
            for ii, tt in enumerate(plist):
                ...
                axr.set_xticks(self.offset_list[list(
                    range(0, ns, self.xtickspace))])
                ...
                axp.set_xticks(self.offset_list[list(
                    range(0, ns, self.xtickspace))])
        ...
        # plot data as an image which can have interpolation
            ...
            for ii, tt in enumerate(plist):
                ...
                if self.text_location is None:
                    txloc = self.offset_list.min() * self.text_xpad
                    tyloc = np.log10(
                        self.period_limits[1] * self.text_ypad) if self.period_limits[0] != 0 else 0
                else:
                    txloc = self.text_location[0]
                    tyloc = self.text_location[1]
