...
from pygmi.mt.mtpyold.analysis.pt import PhaseTensor
from pygmi.mt.mtpyold.analysis.zinvariants import Zinvariants
...
class PlotMultipleResponses(mtpl.PlotSettings):
    ...
    expecting only one type of input --> can be:
        **fn_list** : list of filenames to plot
         ...
         **z_object_list** : list of pygmi.mt.mtpyold.core.z.Z objects

         **res_object_list** : list of pygmi.mt.mtpyold.imaging.mtplot.ResPhase objects

         **tipper_object_list** : list of pygmi.mt.mtpyold.imaging.mtplot.Tipper objects

         **mt_object_list** : list of pygmi.mt.mtpyold.imaging.mtplot.MTplot objects
    ...
    def plot(self, show=True):
        ...
        if self.plot_style == 'all':
            ...
            self.fig = plt.figure(
                self.fig_num, self.fig_size, dpi=self.fig_dpi)
            ...
            for ii, mt in enumerate(self.mt_list):  # type: int, MT
                ...
                if self.phase_limits is None:
                    ...
                    pymax = max(
                        89.9, max([max(rp.phase_xy), max(rp.phase_yx)]))
                ...
                if self._plot_pt == 'y':
                    ...
                    if self.ellipse_colorby == 'phiminang' or \
                        ...
                        raise NameError(self.ellipse_colorby +
                                        ' is not supported')
                ...
                if self.plot_num == 2:
                    # ---------plot the apparent resistivity----------------
                    axr2 = self.fig.add_subplot(
                        gs[0, 1], sharex=axr, sharey=axr)
                    ...
                    # -----Plot the phase-----------------------------------
                    axp2 = self.fig.add_subplot(
                        gs[1, 1], sharex=axr, sharey=axp)
        ...
        if self.plot_style == 'compare':
                ...
                if self._plot_tipper.find('y') == 0:
                    ...
                    for aa in range(nt):
                        ...
                        if self.tipper_limits is None:
                            ...
                            tmax = max(
                                [tyr.max(), tyi.max(), self.tipper_limits[1] - .1]) + .1
                            tmin = min(
                                [tyr.min(), tyi.min(), self.tipper_limits[0] + .1]) - .1
                            if np.isnan(tmax):
                                tmax = 1.0
                            if np.isnan(tmin):
                                tmin = -1.0
            ...
            if self._plot_pt == 'y':
                ...
                for tk in self.axpt.get_xticks():
                    try:
                        tklabels.append(
                            mtpl.labeldict[tk / self.ellipse_spacing])
                        xticks.append(tk)
                    except KeyError:
                        pass
