...
from pygmi.mt.mtpyold.imaging.penetration import get_index, load_edi_files, Depth3D
...
@deprecated("please use get_index() in pygmi.mt.mtpyold.imaging.penetration instead")
...
def get_penetration_depths_from_edi_file(edifile, rholist=['det']):
    ...
    _logger.debug(
        "the scale parameter should be 355.88127 =?= %s", scale_param)
...
def create_penetration_depth_csv(edi_dir, outputcsv, zcomponent='det'):
    ...
    for afile in edi_files:
        ...
        if periods_list0 is None:
            ...
            latlon_dep.append(["Lat", "Lon"] + list(periods)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        ...
        create_penetration_depth_csv(
            edi_dir, "/tmp/%s_MT_pen_depths.csv" % bname)
