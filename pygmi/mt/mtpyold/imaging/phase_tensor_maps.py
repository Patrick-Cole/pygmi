...
import pygmi.mt.mtpyold.imaging.mtcolors as mtcl
import pygmi.mt.mtpyold.imaging.mtplottools as mtpl
import pygmi.mt.mtpyold.analysis.pt as MTpt
from pygmi.mt.mtpyold.utils.mtpylog import MtPyLog
# from pygmi.mt.mtpyold.utils.plot_geotiff_imshow import plot_geotiff_on_axes
...
class PlotPhaseTensorMaps(mtpl.PlotSettings):
    ...
    -------------
        ...
        **z_object** : class pygmi.mt.mtpyold.core.z.Z
                      object of pygmi.mt.mtpyold.core.z.  If this is input be sure the
                      attribute z.freq is filled.  *default* is None

        **mt_object** : class pygmi.mt.mtpyold.imaging.mtplot.MTplot
                        object of pygmi.mt.mtpyold.imaging.mtplot.MTplot
                        *default* is None

        **pt_object** : class pygmi.mt.mtpyold.analysis.pt
                        phase tensor object of pygmi.mt.mtpyold.analysis.pt.  If this is
    ...
    def __init__(self, **kwargs):
        ...
        self.arrow_legend_position = arrow_legend_dict.pop(
    ...
    def plot(self, fig=None, save_path=None, show=True,
        ...
        if (len(raster_dict['lons']) and self.mapscale == 'deg'):
            ...
            assert len(lons) == len(lats) == len(
                vals), 'Lons, Lats and Vals must all have the same length'
        ...
        for ii, mt in enumerate(self.mt_list):
                    ...
                    if self.plot_tipper == 'yri' or self.plot_tipper == 'yr':
                        if ti.mag_real[jj] <= self.arrow_threshold:
                            txr = ti.mag_real[jj] * ascale * \
                                np.sin((ti.angle_real[jj]) *
                                       np.pi / 180 + adir)
                            tyr = ti.mag_real[jj] * ascale * \
                                np.cos((ti.angle_real[jj]) *
                                       np.pi / 180 + adir)
                    ...
                    if self.plot_tipper == 'yri' or self.plot_tipper == 'yi':
                        if ti.mag_imag[jj] <= self.arrow_threshold:
                            txi = ti.mag_imag[jj] * ascale * \
                                np.sin((ti.angle_imag[jj]) *
                                       np.pi / 180 + adir)
                            tyi = ti.mag_imag[jj] * ascale * \
                                np.cos((ti.angle_imag[jj]) *
                                       np.pi / 180 + adir)
            ...
            # ==> print a message if couldn't find the freq
                ...
                _logger.warn('Did not find {0:.5g} Hz for station {1}'.format(
        ...
        **Files Content **
        ...
        for ii in range(nx):
            ...
            try:
                ...
                if pt.phimin[0] is not None:
                    phiminmap[xyloc[ii, 0], xyloc[ii, 1]
                              ] = pt.phimin[0][self.jj]
                    phimaxmap[xyloc[ii, 0], xyloc[ii, 1]
                              ] = pt.phimax[0][self.jj]
                    azimuthmap[xyloc[ii, 0], xyloc[ii, 1]
                               ] = pt.azimuth[0][self.jj]
                    ellipmap[xyloc[ii, 0], xyloc[ii, 1]
                             ] = pt.ellipticity[0][self.jj]
                    betamap[xyloc[ii, 0], xyloc[ii, 1]] = pt.beta[0][self.jj]

                if tp.mag_real is not None:
                    trmap[xyloc[ii, 0], xyloc[ii, 1]] = tp.mag_real[self.jj]
                    trazmap[xyloc[ii, 0], xyloc[ii, 1]
                            ] = tp.angle_real[self.jj]
                    timap[xyloc[ii, 0], xyloc[ii, 1]] = tp.mag_imag[self.jj]
                    tiazmap[xyloc[ii, 0], xyloc[ii, 1]
                            ] = tp.angle_imag[self.jj]
                try:
        ...
        for ii in range(nx):
            ...
            if stationmap[xx, yy] is None or len(stationmap[xx, yy]) < 1:
                ...
                # lon
                tablefid.write(mtpl.make_value_str(
...
if __name__ == "__main__":
    ...
    lons, lats = np.meshgrid(np.linspace(136, 140, 50),
                             np.linspace(-19, -22, 50))
    vals = np.exp(-np.sin(np.radians(lons) * 50) **
                  2 - np.cos(np.radians(lats) * 70)**3)
