...
from pygmi.mt.mtpyold.gui.my_stream import MyStream
...
class ConvertModel2VTK(QtWidgets.QWidget):
    ...
    def set_fn_lists(self):
        ...
        if self.model_type == 'ModEM':
            self._mfn_list = [fn for fn in os.listdir(
                self.cwd) if fn.endswith('.rho')]
            self._rfn_list = [fn for fn in os.listdir(
                self.cwd) if fn.endswith('.dat')]
        elif self.model_type == 'WS':
            self._mfn_list = [fn for fn in os.listdir(
                self.cwd) if fn.find('model') >= 0]
            self._rfn_list = [fn for fn in os.listdir(
                self.cwd) if fn.find('resp') >= 0]
    ...
    def make_vtk_files(self):
        if self.model_type == 'ModEM':
            m_obj = pygmi.mt.mtpyold.modeling.modem.Model()
            ...
            d_obj = pygmi.mt.mtpyold.modeling.modem.Data()
