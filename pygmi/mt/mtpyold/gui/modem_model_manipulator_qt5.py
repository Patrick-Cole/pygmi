...
#        button_layout.addWidget(self.fill_outside_button)
    ...
    def redraw_map(self):
        ...
        self.map_ax.pcolormesh(self.plot_east_map,
                               self.plot_north_map,
                               np.log10(
                                   self.new_res_model[:, :, self.map_index].T),
                               cmap=self.cmap,
                               vmin=self.res_limits[0],
                               vmax=self.res_limits[1])
        if self.data_fn is not None:
    ...
    def redraw_east(self):
        ...
        self.east_ax.pcolormesh(self.plot_north_z,
                                self.plot_z_north,
                                np.log10(
                                    self.new_res_model[:, self.east_index, :]),
                                cmap=self.cmap,
                                vmin=self.res_limits[0],
                                vmax=self.res_limits[1])
    ...
    def redraw_north(self):
        ...
        self.north_ax.pcolormesh(self.plot_east_z,
                                 self.plot_z_east,
                                 np.log10(
                                     self.new_res_model[self.north_index, :, :]),
                                 cmap=self.cmap,
                                 vmin=self.res_limits[0],
                                 vmax=self.res_limits[1])
        ...
        for xx in x_change:
            for yy in y_change:
                if self.model_obj.res_model[xx, self.east_index, yy] < 1E10:
                    self.new_res_model[xx, self.east_index,
                                       yy] = self.res_value
    ...
    def location_pick(self, event):
        ...
        if event.mouseevent.button == 1:
            ...
            try:
                x_index = np.where(self.model_obj.grid_east /
                                   self.scale >= data_point.xdata)[0][0]
            except IndexError:
                return
            try:
                y_index = np.where(self.model_obj.grid_north /
                                   self.scale >= data_point.ydata)[0][0]
            except IndexError:
                return
            dx = np.abs(data_point.xdata -
                        self.model_obj.grid_east[x_index] / self.scale)
            dy = np.abs(data_point.ydata -
    ...
    def smooth_model(self):
        ...
        gauss = (1. / (2 * np.pi * sigma)) * \
            np.exp(-((gx**2) + (gy**2)) / (2 * sigma))
