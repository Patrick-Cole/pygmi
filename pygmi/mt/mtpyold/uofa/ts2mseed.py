...
import pygmi.mt.mtpyold.utils.mseed as MTms
...
def main():
    ...
    if outdir is not None:
        try:
            ...
            print(
                'Cannot generate writable output directory {0} - using generic location "miniSeed" instead'.format(outpath))
            outdir = None
    if outdir is None:
        outpath = op.join(os.curdir, 'miniSeed')
        try:
    ...
    for idx_ipath, inpath in enumerate(lo_indirs):
        ...
        for idx_fn, fn in enumerate(lo_infiles):
            ...
            if MTfh.validate_ts_file(fn) is False:
                print(
                    'Warning - MT ts data file {0} is not valid (check header)!!!'.format(fn))
                # continue
