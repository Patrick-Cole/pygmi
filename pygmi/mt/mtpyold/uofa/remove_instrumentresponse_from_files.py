...
import pygmi.mt.mtpyold.processing.calibration as MTcb
import pygmi.mt.mtpyold.utils.filehandling as MTfh
import pygmi.mt.mtpyold.processing.instrument as MTin
    ...
    for ch in lo_channels:
        ...
        for ta in lo_timeaxes:
            ...
            if (ta[-1] - ta[0]) < (3 * winmax):
                print(
                    'time axis short enough ({0} seconds) - reading all at once'.format(ta[-1] - ta[0]))
                ...
                for idx_t0, t0 in enumerate(lo_windowstarts):
                    ...
                    print('s1 = {0} - {1}, s2 = {2} - {3}, s3 = {4} - {5}\n'.format(
                        t0, lo_windowstarts[idx_t0 + 1] - 1. / samplingrate, lo_windowstarts[idx_t0 + 1], lo_windowstarts[idx_t0 + 2] - 1. / samplingrate, lo_windowstarts[idx_t0 + 2], endtime))
                    ...
                    print('sections ta: ', section1_ta[0], section1_ta[-1], len(section1_ta), section2_ta[0],
                          section2_ta[-1], len(section2_ta), section3_ta[0], section3_ta[-1], len(section3_ta))
                    # continue
                    data = np.array(data)
                    ...
                    while t < tmax:
                        ...
                        if file_open == False:
                            ...
                            outfn = op.join(outdir, outbasename)
                            print(
                                'write header to output file {0} '.format(outfn))
                            outF = open(outfn, 'w')
                            outF.write(headerline)
                            ...
                            if tmax > ta_tmp[-1]:
                                    ...
                                    outF, data2write[
                                        :int(
                                            float(
                                                header['nsamples']))])
                                print(
                                    'write data to output file {0} '.format(outfn))

                                # close the file
                                outF.close()
                                file_open = False
                                print(
                                    ' output file closed : {0} '.format(outfn))
                                datalength = 0
                                ...
                                print('data cut from {0} to {1}'.format(
                                    len(data2write), len(data2write[int(float(header['nsamples'])):])))
                            ...
                            # of the newly opened file:
                                ...
                                if tmax == ta_tmp[-1]:
                                    ...
                                    print(
                                        'written data to file and closed it: {0}'.format(outF.name))
                                ...
                                # file (potentially) misses entries
                                    ...
                                    print('file not full...{0} remains open - {1} of max. {2} samples written '.format(
                                        outF.name, datalength, int(float(header['nsamples']))))
                                    file_open = True
                        ...
                        # section has to be appended there:
                            ...
                            if tmax > ta_tmp[-1]:
                                print('data longer than space left in file...storing first {0} here and cut of the rest ({1}) for further processing...\n'.format(
                                    len(ta_tmp) - datalength, len(data2write) - len(ta_tmp) + datalength))
                            ...
                            # of the open file:
                                ...
                                print('file contains {0} samples - {1} open '.format(
                                    datalength, int(float(header['nsamples'])) - datalength))
                                # check, if by chance this is exactly the
                                # correct number of samples for this file:
                                if tmax == ta_tmp[-1]:
                                    # if so, close it
                                    outF.close()
                                    print(
                                        'written data to file {0}'.format(outfn))
                                    file_open = False
                                    dummy = lo_infiles.pop(0)
                                    dummy = lo_t0s.pop(0)
                                    # actually, the infile list should be empty
