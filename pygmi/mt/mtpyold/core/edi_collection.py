...
import pygmi.mt.mtpyold.imaging.mtplottools as mtplottools
from pygmi.mt.mtpyold.utils.mtpy_decorator import deprecated
from pygmi.mt.mtpyold.utils.matplotlib_utils import gen_hist_bins
from pygmi.mt.mtpyold.utils.mtpylog import MtPyLog
import pygmi.mt.mtpyold.analysis.pt as MTpt
import pygmi.mt.mtpyold.imaging.penetration
...
class EdiCollection(object):
    ...
    def _get_all_periods(self):
        ...
        self._logger.debug("Number of MT Frequencies: %s",
                           len(self.all_frequencies))
        all_periods = 1.0 / \
    ...
    def get_periods_by_stats(self, percentage=10.0):
        ...
        for aper in self.all_unique_periods:
            ...
            if (100.0 * acount) / self.num_of_edifiles >= percentage:
                ...
                self._logger.info(
                    "Period=%s is excluded. it is from stations: %s ", aper, station_list)
        ...
        print("Selected periods %s out of the total %s:" %
    ...
    def create_mt_station_gdf(self, outshpfile=None):
        ...
        pdf = pd.DataFrame(mt_stations, columns=[
                           'StationId', 'Lon', 'Lat', 'Elev', 'UtmZone'])
    ...
    def display_on_image(self):
        ...
        :return: plot object
        """
        # import pygmi.mt.mtpyold.utils.plot_geotiff_imshow as plotegoimg
    ...
    def get_phase_tensor_tippers(self, period, interpolate=True):
        ...
        for mt_obj in self.mt_obj_list:
            ...
            if len(p_index) >= 1:
                ...
                self._logger.warn(" the period %s is NOT found for this station %s. Skipping!!!" % (
    ...
    def create_phase_tensor_csv(self, dest_dir, period_list=None,
        ...
        Implementation based on pygmi.mt.mtpyold.utils.shapefiles_creator.ShapeFilesCreator.create_csv_files
            ...
            for freq in freq_list:
                ...
                for mt_obj in self.mt_obj_list:
                    ...
                    if len(f_index_list) > 1:
                        self._logger.warn(
                            "more than one freq found %s", f_index_list)
                    if len(f_index_list) >= 1:
    ...
    def create_penetration_depth_csv(self, dest_dir, period_list=None, interpolate=False, file_name="penetration_depth.csv"):
        ...
        csv_header = ['station', 'freq', 'lon', 'lat',
                      'pen_depth_det', 'pen_depth_zxy', 'pen_depth_zyx']
        ...
        with open(csvfname, "w", newline="") as csvf:
            ...
            for freq in freq_list:
                ...
                stations, periods, pen_depth_det, latlons = pygmi.mt.mtpyold.imaging.penetration.get_penetration_depth_by_period(
                    self.mt_obj_list, 1.0 / freq)  # whichrho='det')
                stations, periods, pen_depth_zxy, latlons = pygmi.mt.mtpyold.imaging.penetration.get_penetration_depth_by_period(
                    self.mt_obj_list, 1.0 / freq, whichrho='zxy')
                stations, periods, pen_depth_zyx, latlons = pygmi.mt.mtpyold.imaging.penetration.get_penetration_depth_by_period(
                    self.mt_obj_list, 1.0 / freq, whichrho='zyx')

                for iter in range(len(stations)):
                    pdlist.append([stations[iter], freq, latlons[iter][1], latlons[iter]
                                  [0], pen_depth_det[iter], pen_depth_zxy[iter], pen_depth_zyx[iter]])
    ...
    def get_stations_distances_stats(self):
        ...
        for mtobj in self.mt_obj_list:
            mt_stations.append(
                (mtobj.station, mtobj.lat, mtobj.lon, mtobj.utm_zone))

        pdf = pd.DataFrame(mt_stations, columns=[
                           'Station', 'Lat', 'Lon', 'UtmZone'])
        ...
        for i in range(len(pdf)):
            ...
            for j in range(i + 1, len(pdf)):
                ...
                if (dist < 0.004):  # 0.004 is about 400 meters
                    self._logger.info(
                        "Small distances occurred between stations: %s %s", pdf.iloc[i].Station, pdf.iloc[j].Station)
        ...
        self._logger.info(
            "1,2,3,4 5 Percentile distances: %s, %s, %s, %s, %s", q01, q02, q03, q04, q05)
    ...
    def show_obj(self, dest_dir=None):
        ...
        :return:
        """
        print(len(self.all_unique_periods),
              'unique periods (s)', self.all_unique_periods)
        ...
        if dest_dir is None:
            self.plot_stations(savefile=os.path.join(
                self.outdir, 'edi_collection_test.jpg'))
        else:
            self.plot_stations(savefile=os.path.join(
                dest_dir, 'edi_collection_test.jpg'))
