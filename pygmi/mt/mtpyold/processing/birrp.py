...
class BIRRPParameters(object):
    ...
    def _validate_parameters(self):
        ...
        if self._nout not in [2, 3]:
            print(
                'WARNING: Number of outputs {0} not allowed.'.format(self._nout))
            self._nout = 2
        ...
        if self.tbw < 0 or self.tbw > 4:
            print(
                'WARNING: Total bandwidth of slepian window {0} not allowed.'.format(self.tbw))
            self.tbw = 2
            print('  --> setting tbw to {0}'.format(self.tbw))

        if (np.log2(self.nfft) % 1) != 0.0:
            print(
                'WARNING: Window length nfft should be a power of 2 not {0}'.format(self.nfft))
            self.nfft = 2**np.floor(np.log2(self.nfft))
            print('  -- > setting nfft to {0}, (2**{1:.0f})'.format(self.nfft,
                                                                    np.log2(self.nfft)))

        if np.log2(self.nfft) - self.nsctmax < 4:
            print('WARNING: Maximum number of windows {0} is too high'.format(
                self.nsctmax))
        ...
        if self.ilev == 1:
            ...
            if self.nfsect != 2:
                ...
                print('number of frequencies per section is {0}'.format(
                    self.nfsect))
                self.nfsect = 2
        ...
        if len(self.thetae) != 3:
            print('WARNING: Electric rotation angles not input properly {0}'.format(
                self.thetae))
        ...
        if len(self.thetab) != 3:
            print('WARNING: Magnetic rotation angles not input properly {0}'.format(
                self.thetab))
            print('input as north, east, orthogonal rotation')
            self.thetab = [0, 90, 0]
        ...
        if len(self.thetaf) != 3:
            print('WARNING: Field rotation angles not input properly {0}'.format(
                self.thetaf))
...
class J2Edi(object):
    ...
    -------------
        ...
        >>> import pygmi.mt.mtpyold.proceessing.birrp as birrp
    ...
    def get_birrp_config_fn(self):
        ...
        try:
            ...
            print('WARNING: Could not find a birrp_params config file in {0}'.format(
                self.birrp_dir))
            self.birrp_config_fn = None
            return
    ...
    def get_j_file(self, birrp_dir=None):
        ...
        try:
            ...
            print('ERROR: Could not find a .j file in {0}, check path.'.format(
                self.birrp_dir))
            self.j_fn = None
    ...
    def _fill_info(self):
        ...
        for key in sorted(self.birrp_dict.keys()):
            setattr(self.mt_obj.Processing, 'birrp_' +
                    key, self.birrp_dict[key])
...
#        self.mt_obj.Define_measurement.maxchan = ch_count
    ...
    def write_edi_file(self, station=None, birrp_dir=None,
        ...
        This file can be written using pygmi.mt.mtpyold.utils.configfile::
