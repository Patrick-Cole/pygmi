...
def read_winglink_xyzv(input_file, false_easting=0.0, false_northing=0.0):
    ...
    source_points = np.array(
        [arr[:, 0] - false_easting, arr[:, 1] - false_northing, arr[:, 2]]).T
    source_values = arr[:, 3]
...
def mask_interpolate_and_write(output_file,
    ...
    utm_points = nc.transform_3d(
        grid_proj, source_proj, nc.flatten_grid(grid_points))
    ...
    source_points_in_grid_proj = nc.transform_3d(
        source_proj, grid_proj, source_points)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--source-proj', help='EPSG projection code for input file', type=int, default=32753)
    parser.add_argument(
        '--false-easting', help="false easting to correct", type=float, default=500000.0)
    parser.add_argument(
        '--false-northing', help="false northing to correct", type=float, default=10000000.0)
    parser.add_argument(
        '--grid-proj', help='EPSG projection code for output grid', type=int, default=4326)
    parser.add_argument('--grid-corner-1', help="one corner of the output grid in x,y,z format",
                        type=str, default="131.5,-31.0,-4500.0")
    parser.add_argument('--grid-corner-2', help="the other corner of the output grid in x,y,z format",
                        type=str, default="132.5,-30.0,-125.0")
    parser.add_argument('--grid-resolution', help='resolution of the output grid in x,y,z format',
                        type=str, default="0.005,0.005,100.0")
    ...
    source_proj = Proj('epsg:' + str(args.source_proj))
    grid_proj = Proj('epsg:' + str(args.grid_proj))
    main(args.input_file, source_proj, args.false_easting, args.false_northing,
         args.output_file, grid_proj, left, right, resolution)
