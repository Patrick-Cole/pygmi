...
from pygmi.mt.mtpyold.contrib.netcdf import nc
...
def uniform_interior_grid(arr, spacing, mid):
    ...
    units = int((end_points[0] - mid) /
                spacing), int((end_points[1] - mid) / spacing) + 1
...
def main(data_file, model_file, output_file, source_proj=None):
    ...
    source_proj = Proj('epsg:' + str(epsg_code))
    ...
    grid_proj = Proj('epsg:4326')
    # output grid Coordinate system 4326, 4283, 3112
    grid_proj = Proj('epsg:4283')
    # output grid Coordinate system 4326, 4283, 3112
    grid_proj = Proj('epsg:3112')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output-file', default="output.nc",
