...
class ImportCG5(BasicModule):
    ...
    def settings(self, nodialog: bool = False) -> bool:
        ...
        if self.cmb_line.currentText() == self.cmb_station.currentText():
            self.showlog("Your line column cannot be the same as your station column")
            return False

        tmp = [
            self.cmb_line.currentText(),
        ...
        if len(set(tmp)) != len(tmp):
            self.showlog(
                "Unable to import, two of your GPS file "
        ...
        dfmerge = merge_gpsmag(
            self.le_cg5file.text(),
...
def merge_gpsmag(
    ...
    if df_gps.latitude.dtype == "O":
        ...
        try:
            ...
            showlog(
                "You have characters in your latitude"
    ...
    if df_gps.longitude.dtype == "O":
        ...
        try:
            ...
            showlog(
                "You have characters in your longitude"
                " string which could not be converted."
            )
            return False
    ...
    df_gps["station"] = pd.to_numeric(
        df_gps["station"], errors="coerce", downcast="float"
    )

    df_gps["line"] = pd.to_numeric(df_gps["line"], errors="coerce", downcast="float")
