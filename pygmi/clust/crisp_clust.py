...
def crisp_means(
    ...
    edist = np.array(
        [np.sqrt(np.sum(((data - onetmp * cent[j]) ** 2), 1)) for j in range(no_clust)]
    )
    ...
    for i in piter(range(maxit)):  # =1:maxit. loop over all iters
        ...
        if centfix.size > 0:
            ...
            cent[cent_idx == 1] = cent_orig(cent_idx == 1) + centfix(cent_idx == 1)
            cent_idx = cent < (cent_orig - centfix)
            cent[cent_idx == 1] = cent_orig(cent_idx == 1) - centfix(cent_idx == 1)
...
def gdist(data, center, index, no_clust, cltype, cov_constr):
    ...
    if cltype == "k-means":
        ...
        for j in range(no_clust):
            ...
            mat_a = np.dot(
                np.ones([no_datasets, 1]) * mod_idx * dcent.T, dcent / np.sum(mod_idx)
            )
            ...
            if np.linalg.cond(mat_a) > 1e10:
                ...
                mat_a = np.dot(
                    np.dot(ev1, (ed1 * np.eye(no_datasets))), np.linalg.inv(ev1)
                )
        ...
        for j in range(no_clust):
            ...
            mbig = np.linalg.det(mat_a0) ** (1.0 / no_datasets) * np.linalg.pinv(mat_a0)
    ...
    # algorithm
        ...
        for j in range(no_clust):
            ...
            # assume spherical shape of clusters with only one sample
                ...
                mat_a = mat_a0
            mbig = np.linalg.det(mat_a) ** (1.0 / no_datasets) * np.linalg.pinv(mat_a)
