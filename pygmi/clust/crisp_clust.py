...
class CrispClust(BasicModule):
    ...
    def settings(self, nodialog=False):
        ...
        dat_out = crispclust(data, self.cltype, self.min_cluster,
                             self.max_cluster, self.constrain, self.runs,
                             self.max_iter, self.term_thresh, self.init_type,
                             ifiles, self.showlog, self.piter)
...
def crispclust(data, cltype='k-means', min_cluster=5, max_cluster=5,
               cov_constr=0., no_runs=1, max_iter=100,
    ...
    for i in range(no_clust[0], no_clust[1] + 1):
        ...
        if init_type == 'data driven':
            ...
            clidx, clcent, clobj_fcn, clvrc = crisp_means(
                ...
                term_thresh, cltype, cov_constr, showlog, piter)
...
def crisp_means(data, no_clust, cent, centfix, maxit, term_thresh,
                cltype, cov_constr, showlog=print, piter=iter):
    """
    ...
    for i in piter(range(maxit)):  # =1:maxit. loop over all iters
