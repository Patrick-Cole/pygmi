...
# -----------------------------------------------------------------------------
"""Import and export of remote sensing data."""
...
from pygmi.rsense.emit import emit_xarray, xr_to_pygmi
...
class ImportData(BasicModule):
    ...
    def __init__(self, parent=None):
        ...
        self.cb_ensuresutm = QtWidgets.QCheckBox(
            "Ensure WGS84 UTM is for southern hemisphere"
        )
    ...
    def settings(self, nodialog=False):
        ...
        dat = get_data(
            self.ifile, piter=self.piter, showlog=self.showlog, tnames=tnames
        )
...
class ImportBatch(BasicModule):
    ...
    def settings(self, nodialog=False):
        ...
        self.tnames[sensor] = [i.text() for i in self.lw_tnames.selectedItems()]
...
class ImportSentinel5P(BasicModule):
    ...
    def get_5P_data(self, meta):
        ...
        qaval1 = qaval1[dat1 != 9.96921e36]
        pnts1 = pnts1[dat1 != 9.96921e36]
        dat1 = dat1[dat1 != 9.96921e36]
...
class ExportBatch(ContextModule):
    ...
    def setupui(self):
        ...
        ext = (
            "GeoTIFF",
            "GeoTIFF compressed using DEFLATE",
            "GeoTIFF compressed using ZSTD",
            "ENVI",
            "ERMapper",
            "ERDAS Imagine",
        )
...
def export_batch(
    ...
    *,
    ...
    for ifile in ifiles:
        ...
        dat = get_from_rastermeta(ifile, piter=piter, showlog=showlog, tnames=tnames)
...
def get_data(
    ...
    if "AST_" in bfile[:4] and ext == ".tif" and "stack" not in bfile:
        ...
        (ext == ".xml" and ".SAFE" in ifile)
        or ("S2A_" in bfile and ext == ".zip")
        or ("S2B_" in bfile and ext == ".zip")
        or ("S2C_" in bfile and ext == ".zip")
        or (ext == ".safe")
    ):
        dat = get_sentinel2(
            ifile,
...
def get_modisv6(ifile, piter=None, showlog=print, tnames=None, metaonly=False):
    ...
    try:
        ...
        showlog(
            "There seems to be a  problem reading the file. "
    ...
    for ifile2 in subdata:
        ...
        if "Sinusoidal" in wkt:
            ...
            wkt = wkt.replace(
                'DATUM["Not specified (based on custom spheroid)"', 'DATUM["D_Unknown"'
            )
            wkt = wkt.replace('SPHEROID["Custom spheroid"', 'SPHEROID["S_Unknown"')
        ...
        date = datetime.datetime.strptime(datetxt, "%Y-%m-%d").replace(
            tzinfo=datetime.UTC
        )
    ...
    if lulc is not None:
        ...
        dat[-1].data = lulc
        dat[-1].dataid = (
            "LULC: out of earth=7, water=6, barren=5, snow=4, "
...
def get_landsat(ifilet, piter=None, showlog=print, tnames=None, metaonly=False):
    ...
    if ".tar" in ifilet:
        ...
        showlog("Input needs to be tar or _MTL.txt for Landsat. Trying regular import")
        return None
    ...
    for ifile2 in piter(files):
        ...
        if dat[-1].data.mask.size == 1:
            dat[-1].data.mask = (
                np.ma.make_mask_none(dat[-1].data.shape) + dat[-1].data.mask
            )
        ...
        if "L2SP" in ifile2:
            if fext == "LST":
                showlog(f"Converting band {lstband} to Kelvin. Band renamed as {fext}")
...
def get_worldview(ifilet, piter=None, showlog=print, tnames=None, metaonly=False):
    ...
    if "isd" not in dtree:
        showlog(
            "Wrong xml file. Please choose the xml file in the PAN or MUL directory"
        )
        return None
    ...
    if platform == "Multi" and "GE" in satid:
        satbands = {"1": [450, 510], "2": [510, 580], "3": [655, 690], "4": [780, 920]}

        bnum2name = {0: "BAND_B", 1: "BAND_G", 2: "BAND_R", 3: "BAND_N"}
    ...
    if satid == "WV03":
        ...
        Esun = {"BAND_B": 1960.0, "BAND_G": 1853.0, "BAND_R": 1505.0, "BAND_N": 1039.0}
    ...
    for i in piter(dat):
        ...
        szenith = 90.0 - float(dtree["isd"]["IMD"]["IMAGE"]["MEANSUNEL"])
        ...
        idata = i.data
        local = {"idata": idata, "tmp": tmp}
        i.data = ne.evaluate("idata * tmp", local_dict=local)
...
def get_sentinel2_metadata(ifile):
    ...
    for mfile in ["MTD_TL", "MTD_DS"]:
        if ifile.lower()[-3:] == "zip":
            ...
            gmeta = glob.glob(
                os.path.dirname(ifile) + f"\\**\\{mfile}.xml", recursive=True
            )[0]
        ...
        if mfile == "MTD_TL":
            meta["Solar Zenith"] = root.find(
                ".//Tile_Angles//Mean_Sun_Angle//ZENITH_ANGLE"
            ).text
            meta["Solar Azimuth"] = root.find(
                ".//Tile_Angles//Mean_Sun_Angle//AZIMUTH_ANGLE"
            ).text
...
def get_aster_zip(ifile, piter=None, showlog=print, tnames=None, metaonly=False):
    ...
    date = datetime.datetime.strptime(datetxt, "%m%d%Y%H%M%S").replace(
        tzinfo=datetime.UTC
    )
...
def get_aster_metadata(ifile):
    ...
    meta["Solar Azimuth"] = float(
        mdatdict["INVENTORYMETADATA"]["PRODUCTSPECIFICMETADATA"]["SOLAR_AZIMUTH_ANGLE"][
            "VALUE"
        ]
    )
    meta["Solar Zenith"] = 90.0 - float(
        mdatdict["INVENTORYMETADATA"]["PRODUCTSPECIFICMETADATA"][
...
def get_aster_hdf(ifile, piter=None, showlog=print, tnames=None, metaonly=False):
    ...
    zenith = 90.0 - float(meta["Solar_Elevation_Angle"])

    if len(cdate) == 8:
        fmt = "%Y%m%d"
    else:
        fmt = "%Y-%m-%d"
    dte = datetime.datetime.strptime(cdate, fmt).replace(tzinfo=datetime.UTC)
...
def get_ternary(
    ...
    if sunfile is not None:
        ...
        if sdata is None:
            sundata = get_raster(sunfile, metaonly=True, piter=piter, showlog=showlog)
            ...
            sundata = get_raster(sunfile, bounds=bounds, piter=piter, showlog=showlog)
    ...
    newimg = [data[0].copy(), data[0].copy(), data[0].copy(), data[0].copy()]
